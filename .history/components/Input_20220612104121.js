  ...
  return (
    ...
    <div className='flex items-center justify-between pt-2.5'>
        <div className='flex items-center'>
          ...
          <div className='icon cursor-default' onClick={() => datePickerRef.current.click()}>
            <CalendarIcon className='h-[22px] text-[#1d9bf0]' />
            <DatePicker className="hidden" selected={selectedDate} onChange={date => setSelectedDate(date)} ref={datePickerRef}/>
          </div>
        </div>
        </div>
    </div>
