  ...
  return (
    ...
    <div className='flex items-center justify-between pt-2.5'>
        <div className='flex items-center'>
          ...
          <div className='icon' onClick={() => setcal(true)}>
            <CalendarIcon className='h-[22px] text-[#1d9bf0]' />
            { cal && <DatePicker selected={selectedDate} onChange={date => setSelectedDate(date)} calendarContainer={CalendarIcon} ref={datePickerRef}/>}
          </div>
        </div>
        </div>
    </div>
